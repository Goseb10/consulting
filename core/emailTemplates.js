...
export const emailTemplates = {
        ...
        dataArray.forEach((data, index) => {
            if (data.mode === 'unique') {
                ...
                if (isSingle) {
                    html += `
                    <p>
                        ${t('email_nonfiscal_p1_line_1').replace('{mensualite}', titleStr)}<br>
    ...
    enfant: (t, child, res18, res21, res25, formatMonetaire) => {
        const montantStr = child.mode === 'mensuel_init' && child.capitalInitial > 0 
            ? `${formatMonetaire(child.mensualite)} / mois + ${formatMonetaire(child.capitalInitial)} de capital de départ`
            : formatMonetaire(child.mensualite);
        ...
        <p>
            ${t('email_nonfiscal_p1_line_1').replace('{mensualite}', montantStr)}<br>
            ${t('email_nonfiscal_p1_line_2')}<br>
            ${t('email_nonfiscal_p1_line_4')}<br>
            ${t('email_nonfiscal_p1_line_5')}<br>
        </p>
